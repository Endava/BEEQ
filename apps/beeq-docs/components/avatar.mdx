...
The component automatically applies `role="img"` and `aria-label` to its root element. You do not need to add ARIA attributes manually — just provide a descriptive `label` and the component handles the rest.
