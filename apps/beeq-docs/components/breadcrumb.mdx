...
When a `bq-breadcrumb-item` has an `href`, activating it navigates to that URL. The `bqClick` event is cancelable: call `event.preventDefault()` on the event to stop the browser from following the link. Use this to route through a client-side router, prompt the user before leaving an unsaved form, or run analytics before navigating.
...
<script>
  document.querySelector("bq-breadcrumb").addEventListener("bqClick", (event) => {
    // Stop the default link navigation and hand control to your router
...
| Event | Description | Type | Cancelable |
|---|---|---|---|
| `bqBlur` | Handler to be called when item loses focus | `CustomEvent<HTMLBqBreadcrumbItemElement>` | No |
| `bqClick` | Handler to be called when item is clicked. Call `event.preventDefault()` to stop the underlying link navigation when the item is rendered as an `<a>` | `CustomEvent<HTMLBqBreadcrumbItemElement>` | Yes |
| `bqFocus` | Handler to be called when item is focused | `CustomEvent<HTMLBqBreadcrumbItemElement>` | No |
...
<Expandable title="CSS variables" defaultOpen={true}>
  ...
  | `--bq-breadcrumb-item--text-color` | Text color of the breadcrumb item | `var(--bq-text--brand)` |
  | `--bq-breadcrumb-item--text-color-current` | Text color of the current breadcrumb item | `var(--bq-text--secondary)` |
  ...
  | `--bq-breadcrumb-item--separator-color` | Color of the breadcrumb item separator | `var(--bq-ui--tertiary)` |
