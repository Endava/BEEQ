...
`bq-date-picker` submits its current date value with the form. Use `required` and `form-validation-message` when the date is mandatory.
...
| `initialView` | `initial-view` | Calendar view shown when the panel opens | `"days" \| "months" \| "years"` | `"days"` |
...
| `months` | `months` | Number of months displayed for `range` and optionally `multi` | `number` | `1` |
...
| `calendar__month` | Any month button in the month view |
| `calendar__month-selected` | Currently selected month |
| `calendar__years` | Year-view grid |
| `calendar__year` | Any year button in the year view |
...
<Expandable title="CSS variables" defaultOpen={false}>
  ...
  | `--bq-date-picker--header-title-color` | Header title text color (month/year navigation button) | `var(--bq-text--primary)` |
  | `--bq-date-picker--panel-count` | Number of month panels rendered per row (set inline by the component from `months`) | `1` |
  | `--bq-date-picker--row-gap` | Vertical gap between calendar grid rows | `0.125rem` |
  | `--bq-date-picker--validation-color` | Border color when a validation status is set | `var(--bq-stroke--danger)` (switches to `success`/`warning` when set) |
  | `--bq-date-picker--view-cell-background-hover` | Hover background for day/month/year cells | `var(--bq-ui--secondary-hover)` |
  ...
  | `--bq-date-picker--view-cell-text-selected` | Selected text color for day/month/year cells | `var(--bq-text--alt)` |
</Expandable>
