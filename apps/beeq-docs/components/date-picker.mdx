...
<Note>
  All selection types support locale-derived numeric masks. With `locale="en-GB"`, day precision uses `dd/mm/yyyy`; a range uses `dd/mm/yyyy - dd/mm/yyyy`; and multi-date entry uses comma-separated masks. Month and year precision use the matching `mm/yyyy` and `yyyy` masks.
  ...
  A custom `placeholder` is preserved when you provide one. Omit it when you want the mask itself to be visible before the user starts typing.
</Note>
