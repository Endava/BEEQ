...
<Note>
  All selection types expose a fixed, locale-derived numeric mask before entry starts. Its separators stay in place while users replace date segments, so the field never shifts into a free-text layout. With `locale="en-GB"`, day precision uses `dd/mm/yyyy`; a range uses `dd/mm/yyyy - dd/mm/yyyy`; and multi-date entry uses comma-separated masks. Month and year precision use the matching `mm/yyyy` and `yyyy` masks.
  ...
  The locale-derived mask is always shown before entry starts, including when a `placeholder` is provided. On focus, its first segment is selected; typing advances to the next segment, arrow keys move between segments, and deletion restores the segment placeholder. The expected mask is also exposed to assistive technology.
</Note>
...
- Focusing or typing in the input with a keyboard does not open the panel. Press `Alt`+`ArrowDown`, or Tab to the calendar icon and press `Enter` or `Space`, to open the panel. Calendar-trigger activation moves focus into its active date cell; the arrow keys navigate calendar cells and `Escape` closes the panel.
