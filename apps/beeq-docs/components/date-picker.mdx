...
Date pickers combine a calendar with locale-ordered day, month, and year segments. They reduce ambiguity, prevent formatting mistakes, and support single dates, ranges, and multiple dates.
...
| **2** | Date segments | The day, month, and year controls people focus and edit, ordered for the active locale |
...
<Note>
  All selection types use locale-ordered numeric segments. A format hint shows the expected layout before entry starts; for `locale="en-GB"`, day precision uses `dd/mm/yyyy`, month precision uses `mm/yyyy`, and year precision uses `yyyy`. Ranges and multiple dates render separate segment groups.
  ...
  The locale-derived format hint is always shown before entry starts. On focus, one segment is active. Type digits to replace that segment, use `ArrowLeft` and `ArrowRight` to move, `Home` and `End` to reach a group boundary, `ArrowUp` and `ArrowDown` to change a value, and `Backspace` or `Delete` to clear a segment. Paste is not supported.
</Note>

<CodeLivePreview mode="shadow" code={`
  <bq-date-picker name="appointment-date">
...
`}>
  <CodeGroup>
    ...
    <BqDatePicker name="appointment-date">
      <label slot="label">Appointment date</label>
    </BqDatePicker>
    ```
...
<Tip>
  `type="single"` is the default. `range` and `multi` use the same segmented-entry model, so users can type digits or choose dates from the calendar in every selection mode.
</Tip>
...
Use numeric `formatOptions` to set the segment order and separators. The component supports `numeric` or `2-digit` day/month fields with a numeric year. This does not affect the underlying ISO wire value.
...
<Warning>
  Textual or variable-length options such as `month: 'long'`, `weekday`, and `dateStyle` cannot define a numeric segment layout. The component falls back to the locale's numeric layout and logs a development warning.
</Warning>
...
- `bq-date-picker` is form-associated and delegates focus, so it participates in native forms and moves focus into its segmented date field when the component receives focus.
- The segmented field is a labelled `group`; each day, month, and year control is a `spinbutton` with localized limits, value text, and roving tabindex. Range endpoints and multi-date groups have their own accessible labels.
- The segmented field sets `aria-disabled="true"` when `disabled` is `true`. Each segment sets `aria-invalid="true"` for `validationStatus="error"` and native constraint failures such as required, invalid, or out-of-bounds values.
...
- The component announces a completed selection when focus enters the field, after a calendar selection, and announces the current calendar month, year, or year range after previous/next navigation.
- Press `Alt`+`ArrowDown`, or Tab to the calendar icon and press `Enter` or `Space`, to open the panel. Calendar-trigger activation moves focus into its active date cell; the arrow keys navigate calendar cells and `Escape` closes the panel.
- Clicking the segmented field opens the calendar while keeping segment focus. Clicking the calendar icon opens the panel and moves focus into it. Selecting a value from the panel returns focus to the first segment.
- Calendar days and months expose localized full names to assistive technology. Always provide a visible label through the `label` slot so the field and calendar dialog have a programmatic name.
...
| `formatOptions` | *(property only)* | Numeric `Intl.DateTimeFormat` options that control segment order and separators. Textual or variable-length options fall back to the locale numeric layout with a development warning. | `DateTimeFormatOptions` | `undefined` |
...
| `placeholder` | `placeholder` | **Deprecated.** Locale-derived segment format hints always describe the expected entry layout, so this value no longer changes the rendered field. | `string` | `undefined` |
...
| `bqChange` | Fired when the value changes after segment entry or calendar selection | `CustomEvent<{ value: string \| undefined; el: HTMLBqDatePickerElement }>` |
...
| `input` | Segmented date field group |
| `segment` | Editable day, month, or year segment |
| `segment-literal` | Locale-derived separator between date segments |
