...
`bq-icon` keeps its structure intentionally small so it can be reused inside many higher-level components. It loads an SVG and renders it inside a shadow-DOM wrapper.

| Part | Element | Description |
|---|---|---|
| **1** | Base | The internal wrapper that holds the rendered icon |
| **2** | SVG | The injected `<svg>` element used for the icon graphic |
...
<Note>
  The older icon guidance recommends not using small icons as the only interactive target. If the icon triggers an action, make sure the clickable area is comfortably large, or use an icon button pattern instead.
</Note>
...
<Tip>
  The default icon size is `24px`, which is the standard medium size in the older design guidance.
</Tip>
...
`}>
  <CodeGroup>
    ...
    <div className="icon">
      <BqButton className="icon-link" href="https://phosphoricons.com/" target="_blank">
    ...
    <template>
      <div class="icon">
        <BqButton class="icon-link" href="https://phosphoricons.com/" target="_blank">
          <BqIcon name="binoculars-fill" slot="prefix" />
          Explore all the icons available
...
Named icons are loaded from SVG assets. If your app copies BEEQ's icons into a public folder, point the component to that URL with `setBasePath()`.
...
If you want to use an alternative icon library, point `setBasePath()` to a local or remote SVG directory that matches the icon names you plan to use.
...
| Slot | Description |
|---|---|
| None | This component does not expose slots |
