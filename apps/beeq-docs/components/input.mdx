...
>
  <CodeGroup>
    ...
    <!-- Standalone: import { BqIcon, BqInput } from "@beeq/angular/standalone" -->
...
<Expandable title="CSS variables" defaultOpen={false}>
  ...
  |---|---|---|
  | `--bq-input--background-color` | Input background color | `theme(backgroundColor.ui.primary)` |
  | `--bq-input--border-color` | Input border color | `theme(stroke.tertiary)` |
  | `--bq-input--border-color-focus` | Input border color on focus | `theme(stroke.brand)` |
  | `--bq-input--border-radius` | Input border radius | `theme(borderRadius.s)` |
  | `--bq-input--border-style` | Input border style | `solid` |
  | `--bq-input--border-width` | Input border width | `theme(borderWidth.s)` |
  | `--bq-input--gap` | Gap between input content and prefix or suffix | `theme(spacing.xs)` |
  | `--bq-input--helper-margin-top` | Helper text margin top | `theme(spacing.xs)` |
  | `--bq-input--helper-text-color` | Helper text color | `theme(textColor.primary)` |
  | `--bq-input--helper-text-size` | Helper text size | `theme(fontSize.s)` |
  | `--bq-input--icon-size` | Icon size for prefix, suffix, and clear button | `24px` |
  | `--bq-input--label-margin-bottom` | Label margin bottom | `theme(spacing.xs)` |
  | `--bq-input--label-text-color` | Label text color | `theme(textColor.primary)` |
  | `--bq-input--label-text-size` | Label text size | `theme(fontSize.s)` |
  | `--bq-input--padding-end` | End padding | `theme(spacing.m)` |
  | `--bq-input--padding-start` | Start padding | `theme(spacing.m)` |
  | `--bq-input--paddingY` | Vertical padding | `theme(spacing.s)` |
  | `--bq-input--text-color` | Input text color | `theme(textColor.primary)` |
  | `--bq-input--text-placeholder-color` | Placeholder text color | `theme(textColor.secondary)` |
  | `--bq-input--text-size` | Input text size | `theme(fontSize.m)` |
</Expandable>
