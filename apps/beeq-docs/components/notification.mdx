...
<CodeLivePreview
  ...
  <script>
    (() => {
      const root = document.currentScript?.closest('.preview') || document.currentScript?.parentElement;
      if (!root) return;

      const notifications = Array.from(root.querySelectorAll('bq-notification'));
...
<CodeLivePreview
  ...
  <script>
    (() => {
      ...
      const styleId = 'notification-stacked-portal-style';
      ...
      const openBtn = root.querySelector('#notification-stacked-btn');
      if (!openBtn) return;
...
>
  <CodeGroup>
    ...
    export function StackedToastNotification() {
      const handleOpen = useCallback(async () => {
        ...
        document.body.append(notification);
        await (notification as any).toast();
      }, []);
...
<Expandable title="CSS variables" defaultOpen={true}>
  ...
  |---|---|---|
  | `--bq-notification--background` | Notification background color | `theme(backgroundColor.ui.primary)` |
  | `--bq-notification--border-color` | Notification border color | `theme(stroke.transparent)` |
  ...
  | `--bq-notification--content-footer-gap` | Gap between content and footer | `theme(spacing.m)` |
  | `--bq-notification--icon-color-error` | Icon color for error notifications | `theme(fill.danger)` |
  | `--bq-notification--icon-color-info` | Icon color for info notifications | `theme(fill.brand)` |
  | `--bq-notification--icon-color-neutral` | Icon color for neutral notifications | `theme(fill.primary)` |
  | `--bq-notification--icon-color-success` | Icon color for success notifications | `theme(fill.success)` |
  | `--bq-notification--icon-color-warning` | Icon color for warning notifications | `theme(fill.warning)` |
  | `--bq-notification--min-width` | Minimum notification width | `320px` |
  | `--bq-notification--padding` | Notification padding | `theme(padding.m)` |
  | `--bq-notification--title-body-gap` | Gap between title and body | `theme(spacing.s)` |
</Expandable>
