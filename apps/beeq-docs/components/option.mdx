...
<Note>
  Place standalone options in `bq-option-list`, or use them as direct children of `bq-select`. A nested `bq-select` manages the tree semantics, selection state, and checkboxes for its options.
</Note>
...
<CardGroup cols={2}>
  ...
  <Card>
    ...
    * The item will not be managed by `bq-option-list`, `bq-select`, or another component that provides list semantics
  </Card>
</CardGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Nested parent">
    A parent option contains related child options. Use this pattern only inside a multiple `bq-select`, where the Select manages cascading, mixed state, and keyboard tree navigation.
  </Card>
</CardGroup>
...
Each option has an interactive row with a label and optional prefix, suffix, and checkbox controls.

| Part | Element | Description |
|---|---|---|
| **1** | Option row | Interactive surface that contains the option controls and content |
| **2** | Label | The main text content of the option |
| **3** | Prefix | Optional slot for an icon or element rendered before the label |
| **4** | Suffix | Optional slot for an icon or element rendered after the label |
| **5** | Checkbox | Optional selection control for checkbox options |
...
### Nested parent options

Use `expanded` and the `options` slot to define a branch. Inside a nested `bq-select`, do not manage `selected`, `indeterminate`, or `checkbox` on the parent yourself: Select keeps those states in sync with the canonical `value`.

The parent can show a label beside its expand control through `expand-label`. `show-selection-summary` controls the generated summary; customise its complete and partial text with `all-selected-label` and `selected-count-label`. The latter replaces `{count}` with the number of selected selectable descendants.
...
Use `expanded` on the parent and place child options in its `options` slot. Give every option in the branch a unique `value`.

<Note>
  This example focuses on the parent-child relationship. In a nested multiple [Select](/components/select), Select manages checkbox, selected, and indeterminate states.
</Note>
...
`}>
  <CodeGroup>
    ...
    <BqOptionList>
      <BqOption expanded value="frontend">
        Frontend
        <span slot="expand-label">2 levels</span>
...
Set `checkbox` to render a checkbox inside a standalone option list. Use `selected` to reflect an initial checked state.

<Note>
  Inside a flat [Select](/components/select), set `multiple` and `enable-checkboxes` on Select instead. Select applies checkbox presentation to its options. Nested multiple Selects always render checkboxes.
</Note>
...
<CodeLivePreview mode="shadow" code={`
  ...
  <bq-option-list>
    <bq-option checkbox selected value="email-notifications">
      Email notifications
    </bq-option>
    <bq-option checkbox value="product-updates">
      Product updates
    </bq-option>
  </bq-option-list>
`}>
  <CodeGroup>
    ...
    <BqOptionList>
      <BqOption checkbox selected value="email-notifications">
        Email notifications
      </BqOption>
      <BqOption checkbox value="product-updates">
        Product updates
      </BqOption>
    </BqOptionList>
    ```
    ...
    @Component({
      ...
      template: `
        <bq-option-list>
          <bq-option checkbox [selected]="true" value="email-notifications">
            Email notifications
          </bq-option>
          <bq-option checkbox value="product-updates">Product updates</bq-option>
        </bq-option-list>
      `,
    })
    ...
    <template>
      <BqOptionList>
        <BqOption checkbox :selected="true" value="email-notifications">
          Email notifications
        </BqOption>
        <BqOption checkbox value="product-updates">Product updates</BqOption>
      </BqOptionList>
    </template>
    ```
  </CodeGroup>
...
For flat options, `bq-option` sets `role="option"` and exposes `aria-selected`. In a nested `bq-select`, the Select switches the option to `role="treeitem"` and manages the relationship with its tree container.

- `aria-selected` communicates the selected state for a flat option.
- Nested checkbox options expose `aria-checked` as `"true"`, `"false"`, or `"mixed"` for a partial branch.
- Nested parents expose `aria-expanded`, and their child container uses `role="group"` while it is visible.
- `aria-disabled` is always present on the host. It is `"true"` when `disabled` or `hidden` is true.
- `aria-hidden="true"` removes a hidden option from the accessibility tree.

As a developer, you are responsible for:

- **Choosing the right parent** — use `bq-option-list` for a standalone flat list, or `bq-select` for select options. Do not put options in an arbitrary container.
...
- **Keyboard navigation** — for a nested Select, let the Select handle `ArrowUp`, `ArrowDown`, `ArrowRight`, and `ArrowLeft`. Do not intercept these keys in your own handlers.
...
|---|---|---|---|---|
| `allSelectedLabel` | `all-selected-label` | Text shown beside the expand control when all selectable descendants are selected | `string` | `'All selected'` |
| `checkbox` | `checkbox` | If `true`, renders the option as a checkbox option | `boolean` | `false` |
...
| `expanded` | `expanded` | If `true`, displays child options from the `options` slot | `boolean` | `false` |
| `hidden` | `hidden` | If true, the option is hidden | `boolean` | `false` |
| `indeterminate` | `indeterminate` | If `true`, the checkbox represents a partial nested selection | `boolean` | `false` |
| `selected` | `selected` | If true, the option is selected and active | `boolean` | `false` |
| `selectedCountLabel` | `selected-count-label` | Partial-selection summary. Use `{count}` for the selected descendant count | `string` | `'{count} selected'` |
| `showSelectionSummary` | `show-selection-summary` | If `true`, shows the nested selection summary beside the expand control | `boolean` | `true` |
...
| `expand-label` | Optional text inside the expand or collapse control |
| `options` | Nested option items shown when the parent is expanded |
...
| `checkbox-base` | Base wrapper exported from the nested checkbox |
| `checkbox-control` | Control wrapper exported from the nested checkbox |
| `checkbox-input` | Native input exported from the nested checkbox |
| `checkbox-checkbox` | Checkbox indicator exported from the nested checkbox |
| `checkbox-label` | Label exported from the nested checkbox |
| `expand` | Expand or collapse button |
| `expand-button` | Native button exported from the expand control |
| `expand-label` | Label exported from the expand control |
| `selection-summary` | Nested selection summary beside the expand control |
| `options` | Container for nested options |
