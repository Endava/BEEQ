...
<CardGroup cols={3}>
  ...
  <Card title="Multiple select">
    Multiple Select supports independent choices. Add `enable-checkboxes` when checkboxes make the multi-select state easier to scan. Selected items appear as dismissible tags in the input.
  </Card>
  <Card title="Nested multiple select">
    Use nested options for selectable categories and their related choices. Parents cascade selection to their selectable descendants and show a mixed state when only some descendants are selected.
  </Card>
</CardGroup>
...
|---|---|---|
| **1** | Label | The descriptive label rendered above the input control. It can include an optional info icon for additional context and optional/required indicator |
| **2** | Input field | The control area that displays the selected value or placeholder |
| **3** | Placeholder | The hint text shown inside the input when no value is selected |
| **4** | Leading icon | Optional prefix icon displayed at the start of the input field |
| **5** | Helper text (optional) | Helper text below the input providing guidance or validation feedback |
| **6** | Dropdown icon | The chevron caret at the end of the input that indicates the open/closed state |
| **7** | Dropdown panel | The floating container that opens below the input and holds the option list |
| **8** | Menu | The list of selectable `bq-option` items inside the dropdown panel |
| **9** | Scroll | The scrollbar that appears when the option list overflows the panel height |
...
Use nested options when people need to select a category, its related choices, or both. Nesting is available only with `multiple`; single selects render only top-level options and log a browser warning when nested markup is present.

- Nested multi-select always renders checkboxes. You do not need `enable-checkboxes`.
- Selecting a parent selects its selectable descendants. Selecting a child also selects its selectable ancestors.
- A parent becomes indeterminate while only some of its selectable descendants are selected. Disabled and hidden descendants are not changed by a cascade.
- Use a unique `value` for every option in the tree. Duplicate values are reported in the browser console because they make the value and event payload ambiguous.

In the input, nested selections render one tag per selected top-level parent. A partial selection adds the descendant count, such as `Frontend (2)`; a fully selected branch keeps the parent label without a count.
...
Use `multiple` to enable multiple selections. Add `enable-checkboxes` for a flat list when visible checkboxes make the current selections easier to scan. Use `keep-open-on-select` to keep the panel open while the user selects options. The `max-tags-visible` attribute controls how many tags are shown before a `+N` overflow tag appears.
...
Nested options are a multiple-select pattern. Put child options in their parent’s `options` slot and give every option a unique value. The parent below starts expanded with one selected child, so its checkbox is indeterminate and its tag shows the selected descendant count.
...
<CodeLivePreview mode="shadow" code={`
  ...
  <bq-select
    ...
    value='["frontend","frontend:react", "backend", "backend:node"]'
  >
    <label slot="label">Skills</label>
    <bq-option
      all-selected-label="All skills"
      expanded
      selected-count-label="{count} skills selected"
      value="frontend"
    >
      Frontend
      <span slot="expand-label">2 levels</span>
      <bq-option slot="options" value="frontend:react">React</bq-option>
      <bq-option slot="options" value="frontend:vue">Vue</bq-option>
    </bq-option>
    <bq-option value="backend">
      Backend
      <span slot="expand-label">2 levels</span>
      <bq-option slot="options" value="backend:node">Node.js</bq-option>
      <bq-option slot="options" value="backend:dotnet">.NET</bq-option>
    </bq-option>
  </bq-select>
`}>
  <CodeGroup>
    ...
    >
      <label slot="label">Skills</label>
      <BqOption
        allSelectedLabel="All skills"
        expanded
        selectedCountLabel="{count} skills selected"
        value="frontend"
      >
        Frontend
        <span slot="expand-label">2 levels</span>
        <BqOption slot="options" value="frontend:react">React</BqOption>
        <BqOption slot="options" value="frontend:vue">Vue</BqOption>
      </BqOption>
      <BqOption value="backend">
        Backend
        <span slot="expand-label">2 levels</span>
        <BqOption slot="options" value="backend:node">Node.js</BqOption>
        <BqOption slot="options" value="backend:dotnet">.NET</BqOption>
      </BqOption>
    </BqSelect>
    ```
    ...
    ```ts Angular icon="angular" expandable
    import { Component } from "@angular/core";
    import { BqOption, BqSelect } from "@beeq/angular/standalone";

    @Component({
      selector: "app-root",
      standalone: true,
      imports: [BqOption, BqSelect],
      template: `
        <bq-select
          multiple
          keep-open-on-select
    ...
    import { BqOption, BqSelect } from "@beeq/vue";
    </script>
...
#### Set a fully selected branch

Use `value` to set and control the Select's state. It stays a flat array, even when the options are nested.

<Steps>
  <Step title="Add the parent value">
    Include `"frontend"` when the Frontend parent is selected.
  </Step>
  <Step title="Add every selectable descendant">
    Include `"frontend:react"` and `"frontend:vue"` to mark the whole Frontend branch as selected.
  </Step>
</Steps>

The component keeps the parent tag as `Frontend` and shows the `all-selected-label` summary in the open list.

```html HTML icon="html5" expandable
<bq-select
  multiple
  value='["frontend","frontend:react","frontend:vue"]'
>
...
<Warning>
  `selectionTree` is event output, not component input. Do not pass it back into `value`.
</Warning>

<Tip>
  Set `show-selection-summary="false"` on a parent option when the visual summary beside its expand control is not useful in your context.
</Tip>

#### Read a nested selection event

Listen for `bqSelect` when the selection changes. The event gives you two views of the same state:

| If you need to… | Use | Why |
|---|---|---|
| Set the Select value, reset it, submit a form, or call a flat API | `event.detail.value` | This is the canonical flat `string[]` used by the component. |
| Send selected categories and children to a hierarchy-aware API | `event.detail.selectionTree` | This is a selected-only tree, built from the current `value`. |

<Note>
  `selectionTree` exists only for nested multi-select. It is `undefined` for flat and single Selects. Treat it as read-only output; keep using `value` to control the component.
</Note>

```javascript
const skillsSelect = document.querySelector('bq-select');

skillsSelect?.addEventListener('bqSelect', (event) => {
  const { value, selectionTree } = event.detail;

  saveFormState(value);
  if (selectionTree) sendCategoryPayload(selectionTree);
});
```

For example, `event.detail.selectionTree` looks like this after selecting the full Frontend branch and Node.js:
...
[
  {
    "value": "frontend",
    "children": [
      { "value": "frontend:react", "children": [] },
      { "value": "frontend:vue", "children": [] }
    ]
  },
  {
    "value": "backend",
    "children": [
      { "value": "backend:node", "children": [] }
    ]
  }
]
```
...
Keep an accessible label when the layout does not need one visibly. Use visually hidden text in the `label` slot, for example in a toolbar or compact table.
...
<CodeLivePreview mode="shadow" code={`
  ...
  <bq-select placeholder="Placeholder">
    <span class="visually-hidden" slot="label">Choose an account</span>
    <bq-icon name="user-circle" slot="prefix"></bq-icon>
    <span slot="helper-text">
      <bq-icon name="star"></bq-icon>
      Helper text
...
`}>
  <CodeGroup>
    ...
    <BqSelect placeholder="Placeholder">
      <span className="visually-hidden" slot="label">Choose an account</span>
      <BqIcon name="user-circle" slot="prefix" />
      <span slot="helper-text">
        <BqIcon name="star" />
        Helper text
...
Set `disable-search` to keep the option panel usable while blocking any typing in the input. Users can still open the panel, browse options, and pick a value, but they cannot filter the list by typing. Use it when the option set is short enough to scan and you want to prevent free-text input from shrinking the visible options.

<Note>
  The `readonly` attribute currently has the same effect and is now **deprecated** — use `disable-search` instead. In a future major release, `readonly` will prevent changing the selected value rather than disabling filtering.
</Note>
...
<CodeLivePreview mode="shadow" code={`
  ...
  <bq-select placeholder="Placeholder" value="2" disable-search>
    <label slot="label">Select label</label>
    <span slot="helper-text">
      <bq-icon name="star"></bq-icon>
      Helper text
...
`}>
  <CodeGroup>
    ...
    <BqSelect placeholder="Placeholder" value="2" disableSearch>
      <label slot="label">Select label</label>
      <span slot="helper-text">
        <BqIcon name="star" />
        Helper text
...
- **Built in:** the native `<input>` inside the control has `role="combobox"`, `aria-haspopup` (`"listbox"` for flat options or `"tree"` for nested options), `aria-expanded` (toggled when the panel opens and closes), and `aria-controls` pointing to the options container by `id`.
...
- **Built in:** nested multi-select changes the option container to a tree. `ArrowRight` expands a branch or moves to its first child; `ArrowLeft` collapses a branch or returns to its parent. `Enter` and `Space` select the focused option without changing expansion.
- **Built in:** parent tree items expose checked, unchecked, or mixed state as their descendants are selected. The separate expand control has its own accessible label.
- **Your responsibility:** provide text in the `label` slot. The component associates its native `<label>` with the combobox input through `for` and `id`. Use visually hidden text when the layout cannot show a visible label.
...
- **Your responsibility:** give every nested option a unique `value`. Values are used for selection state, form submission, tags, and the `selectionTree` event detail.
...
| `disableSearch` | `disable-search` | If `true`, typing in the input is disabled while option selection remains available | `boolean` | `false` |
...
| `enableCheckboxes` | `enable-checkboxes` | If `true` with `multiple`, flat options render checkboxes. Nested multi-select options always render checkboxes | `boolean` | — |
...
| `readonly` | `readonly` | **Deprecated.** Use `disableSearch` to allow selection without text filtering. In a future major release, `readonly` will prevent changing the selected value instead of disabling filtering | `boolean` | — |
...
| `bqSelect` | Emitted when the selected value changes. Detail includes `value` and `item`; nested multi-select also includes selected paths in `selectionTree` |
