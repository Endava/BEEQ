...
<CodeLivePreview code={`
  <style>
    @scope {
      :scope { gap: var(--bq-spacing-l) !important; }
      bq-select::part(panel) { z-index: 12; }
...
`}>
  <CodeGroup>
    ...
    >
      <label slot="label">Custom filtering</label>
      <bq-option *ngIf="isLoading" disabled>
        <bq-spinner slot="prefix" animation size="small" text-position="right"></bq-spinner>
        Loading...
      </bq-option>
      <ng-container *ngIf="!isLoading">
        <bq-option *ngIf="visibleOptions.length === 0" disabled>
          <bq-icon slot="prefix" name="x-circle"></bq-icon>
          No results found
        </bq-option>
        <bq-option *ngFor="let option of visibleOptions" [value]="option.value">
    ...
    export class AppComponent {
      ...
      handleInput(event: BqSelectCustomEvent<{ value: string }>) {
        event.preventDefault();
        const query = event.detail.value;
...
<Expandable title="CSS variables" defaultOpen={false}>
  ...
  |---|---|---|
  | `--bq-select--background-color` | Select background color | `theme(backgroundColor.ui.primary)` |
  | `--bq-select--border-color` | Select border color | `theme(stroke.tertiary)` |
  | `--bq-select--border-color-focus` | Select border color on focus | `theme(stroke.brand)` |
  | `--bq-select--border-color-disabled` | Select border color when disabled | — |
  | `--bq-select--border-radius` | Select border radius | `theme(borderRadius.s)` |
  | `--bq-select--border-width` | Select border width | `theme(borderWidth.s)` |
  | `--bq-select--border-style` | Select border style | `solid` |
  | `--bq-select--gap` | Gap between select content and prefix/suffix | `theme(spacing.xs)` |
  | `--bq-select--helper-margin-top` | Helper text top margin | `theme(spacing.xs)` |
  | `--bq-select--helper-text-color` | Helper text color | `theme(textColor.primary)` |
  | `--bq-select--helper-text-size` | Helper text font size | `theme(fontSize.s)` |
  | `--bq-select--icon-size` | Size of icons in prefix, suffix, and clear button | `24px` |
  | `--bq-select--label-margin-bottom` | Label bottom margin | `theme(spacing.xs)` |
  | `--bq-select--label-text-color` | Label text color | `theme(textColor.primary)` |
  | `--bq-select--label-text-size` | Label font size | `theme(fontSize.s)` |
  ...
  | `--bq-select--text-color` | Input text color | `theme(textColor.primary)` |
  | `--bq-select--text-size` | Input text font size | `theme(fontSize.m)` |
  | `--bq-select--text-placeholder-color` | Placeholder text color | `theme(textColor.secondary)` |
</Expandable>
