...
|---|---|---|
| **1** | Base | The internal wrapper that groups the dot and text |
| **2** | Circle | The colored circular indicator that reflects the selected status type |
| **3** | Text | The visible label that explains what the status means |
...
<Steps>
  <Step title="Write the label first">
    Start with the text. The label should still make sense on its own, because the dot is only a supporting cue.
  </Step>
  <Step title="Choose one semantic type">
    Pick the type that matches the meaning of the message, such as `success` for a completed task or `danger` for a blocked one. Reusing the same type for different meanings makes scanning harder.
  </Step>
  <Step title="Place it near the item it describes">
    Keep the status close to the row, card, or object it belongs to so users do not have to guess what the label refers to.
  </Step>
</Steps>
...
| Slot | Description |
|---|---|
| *(default)* | The text content of the status component |
