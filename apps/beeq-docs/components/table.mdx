---
title: Table
description: Tables display structured data in rows and columns, making it easy to compare values and identify patterns across a dataset.
---
...
Tables organize information into rows and columns so people can compare values, find records, and take action on individual items. BEEQ provides CSS utility classes for the native HTML `<table>` element rather than a custom web component.

<Note>
  Because the table is a CSS-only pattern, you are responsible for the correct HTML structure: `<table>`, `<thead>`, `<tbody>`, `<tr>`, `<th>`, and `<td>`. The BEEQ styles apply only when the `bq-table` class is present on the `<table>` element.
</Note>
...
<Frame className="px-4 py-4" caption="Table component anatomy">
  <img className="block dark:hidden" src="/components/images/table/table-anatomy-light.svg" alt="BEEQ Table component anatomy" />
  <img className="hidden dark:block" src="/components/images/table/table-anatomy-dark.svg" alt="BEEQ Table component anatomy" />
</Frame>
...
<CardGroup cols={2}>
  ...
  <Card>
    ...
    Apply the compact modifier just to fit more rows, if it makes the content harder to read or interact with.
  </Card>
</CardGroup>
...
<Tip>
  Learn more about [CSS custom properties](/theming/global-css-variables) and [global styling](/theming/styles).
</Tip>
...
<CardGroup cols={2}>
  ...
  <Card horizontal title="Source code" icon="github" href="https://github.com/Endava/BEEQ/tree/main/packages/beeq/src/global/styles">
