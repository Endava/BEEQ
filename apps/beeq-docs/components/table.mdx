...
<CodeLivePreview mode="iframe" height="22rem" removePadding code={`
  <style>
    ...
    .pager__nav { display: flex; align-items: center; gap: var(--bq-spacing-s); }
    .pager__size { display: flex; align-items: center; gap: var(--bq-spacing-s); }
  ...
  <div class="pager">
    <div class="pager__size">
      <span class="pager__status">Rows per page</span>
      <bq-select id="size" value="5" style="min-inline-size: 5rem;">
    ...
    <div class="pager__nav">
      <span class="pager__status" id="status"></span>
      <bq-button id="prev" appearance="secondary" size="small">Previous</bq-button>
      <bq-button id="next" appearance="secondary" size="small">Next</bq-button>
    </div>
  </div>
    ...
    (() => {
      ...
      const render = () => {
        ...
        rows.innerHTML = slice.map(r =>
          \`<tr><td>\${r.id}</td><td>\${r.name}</td><td>\${r.status}</td></tr>\`
        ).join('');
      ...
      prev.addEventListener('bqClick', () => { page -= 1; render(); });
      next.addEventListener('bqClick', () => { page += 1; render(); });
  ...
  <CodeGroup>
    ...
    <script>
      ...
      const render = () => {
        const start = (page - 1) * size;
        const slice = data.slice(start, start + size);
        document.querySelector('#rows').innerHTML = slice
          .map((r) => `<tr><td>${r.id}</td><td>${r.name}</td><td>${r.status}</td></tr>`)
          .join('');
      };

      document.querySelector('#prev').addEventListener('bqClick', () => { page -= 1; render(); });
      document.querySelector('#next').addEventListener('bqClick', () => { page += 1; render(); });
      document.querySelector('#size').addEventListener('bqSelect', (event) => {
        size = Number(event.detail.value);
        page = 1;
        render();
      });
    ...
    export function PaginatedTable({ data }) {
      ...
      const pages = Math.max(1, Math.ceil(data.length / size));
      const rows = useMemo(() => data.slice((page - 1) * size, page * size), [data, page, size]);
      ...
      return (
        <>
          ...
          <BqSelect value={String(size)} onBqSelect={(event) => { setSize(Number(event.detail.value)); setPage(1); }}>
            <BqOption value="5">5</BqOption>
            <BqOption value="10">10</BqOption>
            <BqOption value="25">25</BqOption>
          </BqSelect>
          <BqButton appearance="secondary" size="small" disabled={page <= 1} onBqClick={() => setPage(page - 1)}>Previous</BqButton>
          <BqButton appearance="secondary" size="small" disabled={page >= pages} onBqClick={() => setPage(page + 1)}>Next</BqButton>
        </>
      );
    }
    ```
...
<Note>
  Announce page changes for screen reader users. Move focus to the first cell of the new page, or use an `aria-live="polite"` region to say "Page 2 of 5" after each transition.
</Note>
