...
These steps apply to **every** npm-based BEEQ project, regardless of framework. Complete them once, then continue in your stack’s guide for wrapper packages, icons, and framework-specific setup.

<Note>
  If you haven't chosen a stack yet, compare integration paths on [For developers](/getting-started/for-developers) before continuing here.
</Note>

<Note>
  Using a CDN with no build step? Skip this page and go directly to the [HTML / Web Components guide](/guides/frameworks/html-web-components) — it covers CDN setup end-to-end.
</Note>
...
BEEQ is published as `@beeq/core`. It includes the web components, global stylesheet, icon SVG assets, and shared TypeScript types.
...
## Continue in your framework guide

Open the guide that matches your stack under **Frameworks** in the sidebar, or use the links below:
...
- Review [Theming](/theming) if your project needs customization beyond the default theme.
