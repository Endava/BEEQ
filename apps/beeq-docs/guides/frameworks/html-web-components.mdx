...
<Steps>
  <Step title="Check your project setup" titleSize="h3">
    ...
    <Tip>
      If you followed the [Installation guide](/getting-started/installation), you already have `@beeq/core` installed and the stylesheet loaded. Skip straight to the **Bundler** subsection of Step 2 and start at `defineCustomElements()`.
    </Tip>
  </Step>
...
BEEQ components emit custom events prefixed with `bq` (for example `bqClick`, `bqChange`, `bqSelect`). Prefer these over inferring state only from DOM attributes.
...
<Tip>
  Read structured payloads from `event.detail` — that is where BEEQ exposes the latest component state for custom events.
</Tip>

## TypeScript

Install types with `@beeq/core` and import event detail types from the package:
...
<Columns cols={2}>
  <Card title="Angular" href="/guides/frameworks/angular" icon="angular" iconType="duotone">
    Standalone components, forms, and value accessors.
  </Card>
  <Card title="React" href="/guides/frameworks/react" icon="react" iconType="duotone">
    PascalCase components and `onBq...` event props.
  </Card>
  <Card title="Next.js" href="/guides/frameworks/next" icon="react" iconType="duotone">
    SSR, layout setup, and client-only icon initialization.
  </Card>
  <Card title="Vue" href="/guides/frameworks/vue" icon="vuejs" iconType="duotone">
