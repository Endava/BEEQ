...
## How BEEQ maps into React

<Columns cols={2}>
  <Card title="PascalCase components">
    Import components from `@beeq/react` and use them as `BqButton`, `BqInput`, and `BqCard`.
  </Card>
  <Card title="React prop names">
    Use JavaScript prop names such as `onlyIcon`, `debounceTime`, and `justifyContent` instead of kebab-case HTML attributes.
  </Card>
  <Card title="Custom event handlers">
    BEEQ events map to React props such as `onBqClick`, `onBqInput`, and `onBqChange`.
  </Card>
  <Card title="Slots still use slot">
    Pass `slot` to child elements when placing content into prefix, suffix, and other BEEQ slots.
  </Card>
</Columns>
...
<Columns cols={3}>
  <Card title="Explore components" href="/components/button">
    Start with a real component page to see props, events, slots, and framework examples.
  </Card>
  <Card title="Customize the theme" href="/theming/overview">
    Review theming options if your product needs brand customization.
  </Card>
  <Card title="Open Storybook" href="https://storybook.beeq.design">
