...
| `bqViewChange` | Callback handler emitted when the internal calendar view changes (e.g. from `days` to `months`, or from `years` back to `days`). | `CustomEvent<{ view: "months" \| "days" \| "years"; el: HTMLBqDatePicker2Element; }>` |
