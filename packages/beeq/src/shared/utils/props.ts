...
export type TValidProperty<E, T> = TExtractProp<{ [K in keyof E]: E[K] extends T | undefined ? K : never }>;
...
export const validatePropValue = <T extends E[keyof E], E extends Element>(
  ...
  const propertyValue = element[propertyName as keyof E];
  // Early return if the property value is one of the accepted values
  if (ACCEPTED_VALUES.includes(propertyValue as unknown as T)) return;
  // Override property with fallback value
  element[propertyName as keyof E] = fallbackValue;
